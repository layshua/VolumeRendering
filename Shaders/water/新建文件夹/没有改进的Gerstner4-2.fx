...
sampler FoamSampler= sampler_state  //泡沫纹理采样器
{ 
	texture = <FoamTexture>; 
	magfilter = LINEAR; 
	minfilter = LINEAR; 
...
sampler perlin_point_sampler= sampler_state  //Perlin采样器
{ 
	texture = <perm_2d_tex>; 
	magfilter = POINT; 
	minfilter = POINT; 
...
sampler perlin_point_sampler2= sampler_state  //Perlin采样器
{ 
	texture = <grad3_perm_tex>; 
	magfilter = POINT; 
	minfilter = POINT; 
...
{
	return tex2Dlod(perlin_point_sampler,float4((xy + 0.01f) / 256, 0,0)) * 256;
}
float grad_perm(float x, float3 p)
{
	return dot(tex2Dlod(perlin_point_sampler2, float4((x + 0.01f) / 256, 0.5f,0,0)).xyz * 2 - 1, p);
}
...
VS_OUTPUT VS ( float4 inPos : POSITION,float3 h : TEXCOORD0)
{
	...
	float2 tex_coord = Out.Position.xz / (100* 3.2f);
	
	float noise_height = fBm(tex_coord, 3, lacunarity, gain);
	
	Out.Position.y+=noise_height*60;
	Out.normal=finalNormal;
	Out.HEIGHT=heightsY1.x+heightsY1.y+heightsY1.z+heightsY1.w+heightsY2.x+heightsY2.y+heightsY2.z+heightsY2.w;
        return Out;
};
PS_OUTPUT PS(VS_OUTPUT Input)
{
    ...
    float4 foamColor = tex2D(FoamSampler, perturbatedTexCoords);
	...
	float4 temp = 0;
 	...
 	temp = pow(abs(dot(reflectV,eye_dir)),250 ); 
	...
	float4 allcolor=lerp(reflectionColor,waterbody_color,0.85);
	//if(Input.HEIGHT>1.7)
	//{
	//	if(foamColor.x>0.3f || foamColor.y>0.3f || foamColor.z>0.3f)
	//	{
	//	allcolor=lerp(allcolor,foamColor,(1-(1.7/Input.HEIGHT)));
	//	}
	//}
