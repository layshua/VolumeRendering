...
//float bb=0.38823529;
  ...
  //float gg=0.66274510;
  //float bb=0.64705882;
  float rr=0.639215686;
  float gg=0.611764706;
  float bb=0.580392157;
...
struct VS_OUTPUT
{
    ...
    float4 Color                     :COLOR;
    float2 SpecailValue              :TEXCOORD10;
}; 
struct BufferVS_OUTPUT
{
    float4 Position                  : POSITION;
    float4 ReflectionMapSamplingPos  : TEXCOORD1;
    float2 directionTex              : TEXCOORD2;
    float4 Color                     :COLOR;
};
...
VS_OUTPUTWaterHead VSDirectionWaterHead ( float4 inPos : POSITION,float3 h : TEXCOORD0,float2 direction : TEXCOORD1,float2 waterHead : TEXCOORD2)
{
...
//todo by yg
//,float2 specailvalue:TEXCOORD3)
VS_OUTPUT VSDirection ( float4 inPos : POSITION,float3 h : TEXCOORD0,float2 direction : TEXCOORD1,float4 color:COLOR,float2 specailvalue:TEXCOORD10)
{
	...
	Out.directionTex=direction;
        Out.Color=color; 
	//todo by yg
        Out.SpecailValue=specailvalue;
        return Out;
};
PS_OUTPUT BufferPSDirection(VS_OUTPUT Input)
{
	PS_OUTPUT Out=(PS_OUTPUT)0;



	float tempr=(1-Input.Color.b)*rr+Input.Color.b*r1;
        float tempg=(1-Input.Color.b)*gg+Input.Color.b*g1;
        float tempb=(1-Input.Color.b)*bb+Input.Color.b*b1;
   ...
   // Out.Color = (dullBlendFactor*dullColor + (1-dullBlendFactor) * Out.Color); 
    ...
    //========================================添加水的颜色==========================================
	float x1=Out.Color.r/(Out.Color.r  +  tempr);
	float x2=Out.Color.g/(Out.Color.g  +  tempg);
	float x3=Out.Color.b/(Out.Color.b  +  tempb);
    Out.Color.r = Out.Color.r * 0.23 +  0.77*tempr;
    Out.Color.g = Out.Color.g * 0.23 +  0.77*tempg;
    Out.Color.b = Out.Color.b * 0.23 + 0.77*tempb;
    //Out.Color.r = Out.Color.r * 0.20 +  0.8*tempr;
    //Out.Color.g = Out.Color.g * 0.20 +  0.8*tempg;
    //Out.Color.b = Out.Color.b * 0.20 + 0.8*tempb;
    Out.Color.a = a1;
   ...
   float3 xNormalVector = normalize(finalNormal);
	...
	float4 watercolor=float4(tempr,tempg,tempb,0.8f);
   // Out.Color =  watercolor*(1-fresnelTerm)  +   Out.Color*(fresnelTerm); 
	...
	float4 directionTexColor = tex2D(DirectionSampler,Input.directionTex);
        ...
        float4 resultColor1=float4(Input.Color.r,Input.Color.g,1,Input.Color.a);
        if(Input.Color.b<=0.3)
        {
         Out.Color=resultColor1*0;
        }
        else
       {
         Out.Color =((Out.Color*0.9+diffuse*0.5)*(1-0.2*Input.Color.b)+directionTexColor*0.2*Input.Color.b)*resultColor1;
       }
         //Out.Color =(Out.Color*0.9+diffuse*0.5+specular*0.001)*(1-0.2*Input.Color.a)+directionTexColor*0.2*Input.Color.a
...
PS_OUTPUT PSDirection(VS_OUTPUT Input)
{
	PS_OUTPUT Out=(PS_OUTPUT)0;
        float tempr=(1-Input.Color.a)*rr+Input.Color.a*r1;
        float tempg=(1-Input.Color.a)*gg+Input.Color.a*g1;
        float tempb=(1-Input.Color.a)*bb+Input.Color.a*b1;
   ...
   // Out.Color =  watercolor*(1-fresnelTerm)  +   Out.Color*(fresnelTerm); 
	...
	Out.Color =(Out.Color*0.9+diffuse*0.5+specular*0.001)*(1-0.2*Input.Color.a)+directionTexColor*0.2*Input.Color.a;
        ...
        if(Tributary==1)
        { 
           Out.Color.a =Input.Color.a.x+a1*Input.SpecailValue.y;
        }
...
PS_OUTPUT BufferPS(VS_OUTPUT Input)
{
	PS_OUTPUT Out=(PS_OUTPUT)0;
   ...
   // Out.Color =  watercolor*(1-fresnelTerm)  +   Out.Color*(fresnelTerm); 
	...
	Out.Color =(Out.Color*0.9+diffuse*0.5+specular*0.1)*Input.Color;
...
PS_OUTPUT PS(VS_OUTPUT Input)
{
   ...
   // Out.Color =  watercolor*(1-fresnelTerm)  +   Out.Color*(fresnelTerm); 
	...
	Out.Color =Out.Color*0.9+diffuse*0.5+specular*0.1;
...
PS_OUTPUT PSDirectionWaterHead(VS_OUTPUTWaterHead Input)
{
	PS_OUTPUT Out=(PS_OUTPUT)0;
...
technique WaterRenderDirectionHead
{   pass P0
      { 
           VertexShader = compile vs_3_0 VSDirectionWaterHead();
           PixelShader  = compile ps_3_0 PSDirectionWaterHead();

       }
      
}
technique WaterRenderDirection
{   
    pass P0    
       {   VertexShader = compile vs_3_0 VSDirection();
           PixelShader  = compile ps_3_0 PSDirection();
       } 
      

}
technique BufferWaterRenderDirection
{   
    pass P0    
       {   VertexShader = compile vs_3_0 VSDirection();
           PixelShader  = compile ps_3_0 BufferPSDirection();
       } 
...
technique BufferWaterRender
{   
    pass P0    
       {   VertexShader = compile vs_3_0 VSDirection();
           PixelShader  = compile ps_3_0 BufferPS();
       } 
